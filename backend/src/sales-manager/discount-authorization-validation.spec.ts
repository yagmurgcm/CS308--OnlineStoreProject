...
describe('Sales Manager Discount Tests', () => {
  ...
  it('blocks non-sales-manager users from applying discounts', async () => {
    productStore = [
      { id: 1, name: 'Wallet', price: 75, discountRate: 0, discountedPrice: null } as Product,
    ];
    await setupApp('CUSTOMER');

    await request(app.getHttpServer())
      .post('/sales-manager/discounts')
      .send({ productIds: [1], discountRate: 10 })
      .expect(403);
  });

  it('rejects invalid discount rates sent by a sales manager', async () => {
    productStore = [
      { id: 2, name: 'Bag', price: 200, discountRate: 0, discountedPrice: null } as Product,
    ];
    await setupApp('SALES_MANAGER');

    await request(app.getHttpServer())
      .post('/sales-manager/discounts')
      .send({ productIds: [2], discountRate: 150 })
      .expect(400);

    await request(app.getHttpServer())
      .post('/sales-manager/discounts')
      .send({ productIds: [2], discountRate: -10 })
      .expect(400);
  });
});
