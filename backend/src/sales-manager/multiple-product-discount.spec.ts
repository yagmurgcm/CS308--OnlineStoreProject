...
describe('Sales Manager Discount Tests', () => {
  ...
  it('applies a discount to multiple selected products and leaves others unchanged', async () => {
    productStore = [
      { id: 1, name: 'Coat', price: 100, discountRate: 0, discountedPrice: null } as Product,
      { id: 2, name: 'Boots', price: 50, discountRate: 0, discountedPrice: null } as Product,
      { id: 3, name: 'Scarf', price: 30, discountRate: 0, discountedPrice: null } as Product,
    ];
    await setupApp('SALES_MANAGER');

    const response = await request(app.getHttpServer())
      .post('/sales-manager/discounts')
      .send({ productIds: [1, 2], discountRate: 10 })
      .expect(201);

    expect(response.body).toMatchObject({ updatedCount: 2, discountRate: 10 });

    const coat = productStore.find((p) => p.id === 1)!;
    const boots = productStore.find((p) => p.id === 2)!;
    const scarf = productStore.find((p) => p.id === 3)!;

    expect(coat.discountedPrice).toBe(90);
    expect(boots.discountedPrice).toBe(45);
    expect(scarf.discountRate).toBe(0);
    expect(scarf.discountedPrice).toBeNull();
  });
});
