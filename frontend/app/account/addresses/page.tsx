    ...
    return (
      <div className="space-y-6 max-w-3xl">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Input label="Zip/Postcode *" name="zip" value={form.zip} onChange={handleChange} />
        ...
        <div className="flex gap-3">
          <button className="px-5 py-2 bg-black text-white rounded hover:bg-gray-800" onClick={handleSave}>
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {addresses.map((a, i) => (
          <div key={i} className="relative border p-4 rounded-md shadow-sm bg-white">
            <div className="absolute right-2 top-2 flex gap-2">
              <button className="text-blue-600 hover:text-blue-800 text-sm" onClick={() => handleEdit(i)}>✏️</button>
              <button className="text-red-600 hover:text-red-800 text-sm" onClick={() => handleDelete(i)}>🗑️</button>
            </div>

            <p className="text-lg font-semibold">{a.title}</p>
            <p className="font-medium mt-1">{a.firstName} {a.lastName}</p>
...
function Input({ label, name, value, onChange }: any) {
  return (
    <label className="flex flex-col text-sm">
      {label}
      <input name={name} value={value} onChange={onChange} className="mt-1 border border-gray-300 rounded px-3 py-2" />
    </label>
  );
}
...
function Select({ label, name, value, onChange, options }: any) {
  return (
    <label className="flex flex-col text-sm">
      {label}
      <select name={name} value={value} onChange={onChange} className="mt-1 border border-gray-300 rounded px-3 py-2">
